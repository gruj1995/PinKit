...
public struct DateUtility {
    ...
    ///   - month: 月: Int
    ///   - day: 日: Int
    public func getDateFrom(year: Int, month: Int, day: Int, hour: Int = 0, minute: Int = 0, second: Int = 0) -> Date? {
    ...
    public func getDateComponentsFrom(date: Date, dateComponents: Set<Calendar.Component> = [.year, .month, .day, .hour, .minute, .second]) -> DateComponents {

           return calendar.dateComponents(dateComponents, from: date)
    }

    /// 比較兩個日期差異的天數
    public func daysBetweenDate(from: Date, to: Date) -> Int {
        let components = calendar.dateComponents([.day], from: from, to: to)
        return components.day ?? 0
    }

    /// 判斷某日期是否在距今的指定天數內
    public func isDateWithinDays(date: Date, withinDays: Int) -> Bool {
    ...
    public func getDateFromMs(ms: Int64) -> Date {
